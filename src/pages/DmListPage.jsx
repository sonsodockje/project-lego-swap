...
export default function DmListPage() {
                ...
                const loadedChatRooms = Object.entries(data)
                    ...
                    .filter(chatRoom => {
                            ...
                            if (parts.length === 3) {
                                ...
                                console.log("Extracted user IDs:", user1Id, user2Id);
                                console.log("Current user ID:", currentUser.uid);
                                const isParticipant = user1Id === currentUser.uid || user2Id === currentUser.uid;
                                console.log("Is participant:", isParticipant);
                                return isParticipant;
                            }
                            console.log("ChatRoom ID does not match expected format (productId_user1Id_user2Id).");
                            return false;
                        });
    ...
    if (loading) {
        return <div className="text-center mt-8">채팅방 목록을 불러오는 중...</div>;
    }
    ...
    if (chatRooms.length === 0) {
        return <div className="text-center mt-8">참여하고 있는 채팅방이 없습니다.</div>;
    }
                ...
                {chatRooms.map((chatRoom) => (
                    ...
                    >
                        <div className="flex gap-5">
                            <div className="">
                            <img className='w-20 h-20 rounded-md object-cover' src={chatRoom.chatImgUrl} alt="" />
                        </div>
                        <div className="">
                            <div className="flex w-full   justify-between items-center mb-2">
                                <h2 className="text-lg font-semibold">{chatRoom.chatRoomName}</h2>
                        ...
                        <p className="text-gray-700">
                            마지막 메시지: {chatRoom.lastMessage || '메시지 없음'}
                        </p>
                        </div>
                        </div>
